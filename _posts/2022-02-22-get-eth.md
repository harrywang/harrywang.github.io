...
- setup Chrome drive with options. **NOTE** when running headless mode, the user_agent must be specified otherwise a reCAPTCHA check will show up and fail. You can use `driver.get_screenshot_as_file("screenshot.png")` to take screenshots in headless model to help with debugging.
