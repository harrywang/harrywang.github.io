---
layout: post
title: Get Testing Ethers using Selenium
tags: tutorial open-source
permalink: get-eth
---

I need some testing ethers for Rinkeby and found out that the faucet is not working (I also don't like that you have to tweet to get the ethers):
...
I wrote a program to control the browser (via Selenium) to get the testing ethers and scheduled the program to run every 24 hours using AWS EC2. Now, I am getting some testing ethers automatically every day :).
...
Now, you can close the terminal and automatically get 0.1 ether (or more if multiple addresses are used) every day :). Make sure to delete the cron task when you get enough testing ethers.